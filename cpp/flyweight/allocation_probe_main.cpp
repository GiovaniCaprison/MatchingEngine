// The allocation proof in this language (NFR-4.3): the allocator refuses after initialisation, and
// refusal here is counting rather than crashing so the probe can say how many times it was asked.
// This rung's initialisation includes one command: the instrument definition is what sizes the
// ladder (FR-1.1), so the first command of the log runs while the allocator still says yes, and
// every command after it runs against an allocator that records each request, where one request is
// a failure. Events go into a fixed buffer and are dropped, because the proof is about the
// engine's memory and capturing output would put the harness's allocation on the engine's bill.
...
int main(const int count, char** arguments) {
  ...
  matching::flyweight::FlyweightEngine engine(events);
