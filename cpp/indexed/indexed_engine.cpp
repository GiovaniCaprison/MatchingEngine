...
void IndexedEngine::onCommand(char* buffer, const std::size_t offset, const std::size_t length) {
...
void IndexedEngine::enter(protocol::NewOrder& newOrder) {
...
void IndexedEngine::admit(const OrderPtr& order) {
  if (order->stop()) {
    triggers_.add(order);
...
void IndexedEngine::settle(const OrderPtr& order) {
...
void IndexedEngine::match(const OrderPtr& taker) {
...
bool IndexedEngine::prevented(const OrderPtr& taker, const OrderPtr& resting) {
...
void IndexedEngine::take(const OrderPtr& taker, const OrderPtr& resting) {
...
void IndexedEngine::takeExactly(const OrderPtr& taker, const OrderPtr& resting,
...
void IndexedEngine::proRata(const OrderPtr& taker, const std::int64_t price) {
...
void IndexedEngine::fireTriggers() {
  if (lastExecuted_ == 0) {
...
// (FR-4.1, FR-4.2)
void IndexedEngine::cancel(const std::uint64_t clientOrderId, const std::uint32_t participantId) {
...
void IndexedEngine::replace(const std::uint64_t clientOrderId, const std::uint32_t participantId,
...
void IndexedEngine::massCancel(const std::uint64_t clientOrderId,
...
void IndexedEngine::changeState(const SessionState::Value entering) {
...
std::int64_t IndexedEngine::cross(const OrderPtr& buy, const OrderPtr& sell,
...
void IndexedEngine::reveal(const OrderPtr& order, const bool replenishes,
...
std::vector<OrderPtr> IndexedEngine::willing(const Side side, const std::int64_t price) const {
...
void IndexedEngine::reportIndicative() {
  if (!callPhase(state_)) {
    return;
  }
  const Auction::Uncrossing uncrossing = Auction::uncrossing(book_, reference_);
...
RejectReason::Value IndexedEngine::refusalFor(protocol::NewOrder& newOrder) const {
  if (state_ == SessionState::CLOSED) {
...
RejectReason::Value IndexedEngine::refusalForPrice(const std::int64_t price) const {
...
RejectReason::Value IndexedEngine::refusalForTriggerPrice(const std::int64_t price) const {
  return refusalOnTheInstrument(price);
}

// What any price on this instrument has to satisfy, trigger prices included.
RejectReason::Value IndexedEngine::refusalOnTheInstrument(const std::int64_t price) const {
  if (price <= 0) {
...
RejectReason::Value IndexedEngine::refusalFromTheBook(
...
RejectReason::Value IndexedEngine::refusalForReplace(const Order& resting,
                                                     const std::int64_t quantity,
