...
void LeanEngine::match(const OrderPtr taker) {
  while (taker->remaining() > 0) {
    ...
    const std::int64_t shownBefore = resting->remaining();
    taker->take(quantity);
...
void LeanEngine::replace(const std::uint64_t clientOrderId, const std::uint32_t participantId,
  ...
  const OrderPtr replacement = pool_.acquire();
  replacement->init(resting->id(), resting->clientOrderId(), resting->participantId(),
                    resting->side(), resting->pricing(), resting->timeInForce(), price, remainder,
                    ++arrival_, resting->executed());
  pool_.release(resting);
  if (matching()) {
    match(replacement);
  }
