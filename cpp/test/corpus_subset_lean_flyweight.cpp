// The corpus, restricted to what the lean engine is, replayed in this language against rung
// three's lean twin. The filter is the Java subset test's filter word for word: a fixture is
// inside the shared remit when no NEW line carries a qualifier, no session enters a call phase,
// and the instrument allocates price-time.
...
TEST_CASE("the corpus subset inside the shared remit passes against the lean-flyweight engine") {
  int inside = 0;
  for (const conformance::Fixture& fixture : conformance::corpusFixtures()) {
    ...
    const conformance::CorpusRunner::Result result =
        conformance::CorpusRunner::run(fixture, [](api::EventPublisher& events) {
          return std::make_unique<lean::flyweight::LeanEngine>(events);
        });
