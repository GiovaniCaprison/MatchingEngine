// The whole specification against rung one in this language. The same 77 files every engine
...
TEST_CASE("every fixture in the corpus passes against rung one in this language") {
  ...
  for (const conformance::Fixture& fixture : fixtures) {
    const conformance::CorpusRunner::Result result = conformance::CorpusRunner::run(
        fixture,
        [](api::EventPublisher& events) { return std::make_unique<pooled::PooledEngine>(events); });
