...
/**
 * The rung's cross-language differential: the Java and C++ pooled engines fed identical bytes,
 * outputs diffed. With each of them also held byte identical to the naive arbiter, every engine so
 * far agrees pairwise, and a measured difference anywhere in the matrix is a difference of runtime
...
class TwinDifferentialTest {
  ...
  @DisplayName("the two pooled engines emit byte identical output from identical input")
  @ParameterizedTest(name = "seed {0}")
  @ValueSource(longs = {11, 20260826})
  void the_twins_agree(final long seed) throws Exception {
    final Path binary = binary();
    assumeTrue(Files.isExecutable(binary), "no C++ pooled engine is built, nothing to diff");
    ...
    final byte[] ours = DifferentialReplay.replay(log, new PooledEngineFactory());
  ...
  private static Path binary() {
    ...
    while (candidate != null) {
      final Path binary = candidate.resolve("cpp/build/pooled/differential-pooled");
      if (Files.exists(binary)) {
        return binary;
      }
      candidate = candidate.getParent();
    }
    return Path.of("cpp/build/pooled/differential-pooled");
  }
}
