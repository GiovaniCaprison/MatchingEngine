package io.github.giovanicaprison.matching.pooled.boundary;

import static org.assertj.core.api.Assertions.assertThat;
import static org.junit.jupiter.api.Assumptions.assumeTrue;

import io.github.giovanicaprison.matching.flow.CommandLog;
import io.github.giovanicaprison.matching.flow.FlowGenerator;
import io.github.giovanicaprison.matching.flow.FlowParameters;
import java.nio.file.Files;
import java.nio.file.Path;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.io.TempDir;

/**
 * The other language's half of the allocation proof (NFR-4.3): the C++ probe's allocator counts
 * every request made while the session replays, and one request is a failure. The Java arm proves
 * the same claim under Epsilon, so the two engines are held to the same standard by the mechanism
 * native to each runtime.
 */
class TwinAllocationProofTest {

  @TempDir private Path exchanged;

  @DisplayName("NFR-4.3 the C++ engine never asks the allocator once the session is running")
  @Test
  void the_twin_allocates_nothing() throws Exception {
    final Path binary = binary();
    assumeTrue(Files.isExecutable(binary), "no C++ allocation probe is built, nothing to prove");

    final CommandLog log = FlowGenerator.generate(FlowParameters.withAuctions(31, 200_000, 2_000));
    final Path logFile = exchanged.resolve("commands.log");
    log.writeTo(logFile);

    final Process process =
        new ProcessBuilder(binary.toString(), logFile.toString()).redirectErrorStream(true).start();
    final String said = new String(process.getInputStream().readAllBytes());
    assertThat(process.waitFor())
        .as("the probe's allocator was asked during the session: %s", said)
        .isZero();
  }

  private static Path binary() {
    Path candidate = Path.of("").toAbsolutePath();
    while (candidate != null) {
      final Path binary = candidate.resolve("cpp/build/pooled/allocation-probe-pooled");
      if (Files.exists(binary)) {
        return binary;
      }
      candidate = candidate.getParent();
    }
    return Path.of("cpp/build/pooled/allocation-probe-pooled");
  }
}
