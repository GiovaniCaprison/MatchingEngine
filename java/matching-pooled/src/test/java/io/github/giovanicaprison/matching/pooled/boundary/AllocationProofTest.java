 ...
 * per-run allocation budget. The pooled engine replays a long session inside it and exits cleanly.
 * The control arm is the same probe holding the naive engine, which allocates per command and has
 * to die there; without it a passing run would also be consistent with a budget too generous to
 * catch anything.
 */
class AllocationProofTest {
  ...
  @DisplayName("NFR-4.3 the pooled engine finishes a session under a collector that never collects")
  @Test
  void the_steady_state_allocates_nothing() throws Exception {
    assertThat(probe("pooled"))
        .as("the pooled engine outlived the budget only if its steady state allocates nothing")
        .isZero();
  }
  ...
  private int probe(final String engine) throws Exception {
    ...
    final Process process =
        new ProcessBuilder(
                ...
                "io.github.giovanicaprison.matching.pooled.EpsilonProbe",
                logFile.toString(),
                engine)
