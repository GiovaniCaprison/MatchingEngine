...
/**
 * Rung two's lean twin: limit and market orders, price-time, on the pooled book.
 *
 ...
 *
 * <p>On the shared remit this engine, the full pooled rung, and every arm below them are byte
...
public final class LeanEngine implements MatchingEngine {
  ...
  private void match(final Order taker) {
    while (taker.remaining() > 0) {
      ...
      final long shownBefore = resting.remaining();
      taker.take(quantity);
  ...
  private void replace() {
    ...
    final Order replacement = pool.acquire();
    replacement.init(
        resting.id(),
  ...
  private void massCancel() {
    ...
    gathered.clear();
    book.of((int) massCancel.participantId(), gathered);
