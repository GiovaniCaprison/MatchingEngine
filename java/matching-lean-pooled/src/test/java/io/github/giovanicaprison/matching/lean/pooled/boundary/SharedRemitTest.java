...
import io.github.giovanicaprison.matching.lean.pooled.LeanEngineFactory;
...
class SharedRemitTest {
  ...
  @DisplayName("the full pooled engine and its lean twin agree on the shared remit, to the byte")
  ...
  void the_two_arms_agree(final long seed) {
    ...
    final byte[] full = DifferentialReplay.replay(log, new PooledEngineFactory());
    final byte[] lean = DifferentialReplay.replay(log, new LeanEngineFactory());

    final int difference = Arrays.mismatch(full, lean);
    assertThat(difference)
        .as(
            "the streams part company at byte %d of %d (pooled) and %d (lean), so the comparison"
