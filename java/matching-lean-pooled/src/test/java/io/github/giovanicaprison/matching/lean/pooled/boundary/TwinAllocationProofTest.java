 ...
 * allocator counts every request made while the session replays, and one request is a failure.
 */
class TwinAllocationProofTest {

  @TempDir private Path exchanged;

  @DisplayName("NFR-4.3 the C++ lean twin never asks the allocator once the session is running")
  @Test
  ...
  private static Path binary() {
    ...
    while (candidate != null) {
      final Path binary = candidate.resolve("cpp/build/lean-pooled/allocation-probe-lean-pooled");
      if (Files.exists(binary)) {
        return binary;
      }
      candidate = candidate.getParent();
    }
    return Path.of("cpp/build/lean-pooled/allocation-probe-lean-pooled");
  }
}
