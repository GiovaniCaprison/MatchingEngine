package io.github.giovanicaprison.matching.lean.boundary;
...
import io.github.giovanicaprison.matching.lean.LeanEngineFactory;
...
class SharedRemitTest {
  ...
  @DisplayName("the full engine and the lean one agree on the shared remit, to the byte")
  ...
  void the_two_arms_agree(final long seed) {
    ...
    final byte[] full = DifferentialReplay.replay(log, new NaiveEngineFactory());
    final byte[] lean = DifferentialReplay.replay(log, new LeanEngineFactory());

    final int difference = Arrays.mismatch(full, lean);
    assertThat(difference)
        .as(
            "the streams part company at byte %d of %d (full) and %d (lean), so the comparison"
