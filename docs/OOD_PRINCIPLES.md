...
* Why: the failure it prevents. A rule you cannot justify is a rule nobody will keep.
...
> A note on "OOD" in a latency-critical system. A lot of what passes for
> object-oriented design is actively harmful on a hot path: deep hierarchies,
> polymorphism as the default extension mechanism, "tell don't ask" applied to every
> field. Virtual dispatch defeats inlining, encapsulation-by-getter defeats the cache,
> and abstraction layers defeat the JIT's ability to see the whole computation. What
> survives is the part of OOD that was always the valuable part: clear ownership of
> state, narrow interfaces, and making illegal states unrepresentable. That is the
> tradition this document draws on. Where classical OOD conflicts with the hardware,
> the hardware wins on the hot path and classical OOD wins at the edges. OOD-3 defines
> where that border falls, so the choice is never made ad hoc.
...
| [OOD-3](#ood-3-the-coreedge-border) | The core/edge border | enforced |
...
* Compiler, again: "no caller outside `com.imc.me.book..` may invoke a mutator" needs no
  ArchUnit rule, because package-private makes a violation fail to compile. This is the
  concrete meaning of NFR-4.1.
...
simple-but-slow option. It is the faster one. A lock-free single writer keeps the book
...
Here. `TreeMapBookSide` uses plain `HashMap` and `TreeMap` rather than concurrent
variants, and that is a deliberate assertion of this principle. Parallelism, when it
...
and mutable state. There is exactly one entity type in this system, and it lives in the
...
Two obligations follow from this, both now met: every failure result must carry enough
...
Nobody will ever `switch` over book implementations, because that would be a type check on the
...
Both phases live in `TreeMapOrderBook.submit`, as two switches on `OrderType` with no
`default` arm. Phase 1 exists because of FOK: fill-or-kill cannot be a remainder policy,
since by the time the remainder is known you have already traded and cannot untrade. FOK
needs a read-only *probe* over the opposing side, and because that is the same crossing
logic the walk uses, it belongs on `Matcher` (`long fillableQty(aggressor, opposing)`)
rather than in the book. Two copies of a crossing check that can drift apart is the bug
...
(`List.copyOf`) makes it worse: it allocates a second time to buy immutability the core did
not need. A returned collection cannot satisfy both "safe API" and "zero allocation", so
the way out is to invert the direction of data flow. This is the standard idiom in LMAX, Aeron, Chronicle, and every low-latency
JVM system.
...
The `Trade` record still exists and is still valuable as an edge type. A
...
This is also the real fix for API-11.1, which failed until the sinks landed. That rule is
...
`List`. That satisfies API-11.1 and FR-5.5 honestly, because the type is genuinely immutable
rather than immutable only while you trust the constructor.

A known future collision. `submit` returning a `SubmitResult` object also allocates,
once per order. The lowest-latency engines do not return from the write path at all; they
...
* ArchUnit: `no_streams_on_the_hot_path` stops `matching..` or `book..` referencing
  `java.util.stream..`. A stream on the hot path is an allocation cascade.
...
Enforcement.
* ArchUnit: `no_floating_point_in_the_core` and `no_floating_point_fields` between them
  stop any use of `double`, `float`, `Double`, `Float` or `BigDecimal`. These are cheap,
  absolute rules, and once green they cannot regress.
...
`sequence`. `Trade` carries that field, which is what makes NFR-1.1/1.2 provable by
comparing values rather than by comparing whole collections positionally, and what makes an
audit trail reconstructable at all.

Enforcement.

* Test: NFR-1.1 and NFR-1.2 in the golden layer, replay identical input, assert identical
  trades and identical final book.
* ArchUnit: `no_clock_or_randomness_in_the_core` bans `java.time`, `Random`, `UUID` and
  `Math` outright. Clock and randomness are the two ways determinism dies quietly.
...
FR-5.4 as written ("incl remaining qty"), and it now carries both.
...
(the JIT sees one receiver type and devirtualises), so the cost is mainly comprehension, but the moment a third implementation appears, OOD-8's megamorphic wall applies.
