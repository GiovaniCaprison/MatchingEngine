...
per book** — not because threading is hard, but because a deterministic, lock-free,
cache-resident single writer is *faster* than coordinating threads on shared state (the
...
The test tree (`src/test/...`) is organised by test *layer*, not by these packages —
see `TESTING.md`.
...
1. **Walk the opposing book from the best price inward.** For a buy, best ask = lowest
   ...
   naturally crosses every level until liquidity runs out — no special case needed (FR-3.1).
2. **Within a level, match FIFO** from the first real order. Consume
   ...
   order's price** — price improvement accrues to the aggressor (FR-3.5). The trade record
...
**Reading results:** report **p50 / p99 / p99.9 / max latency**, never the mean — the tail is
...
each future rewrite — you'll have *numbers* proving the step helped.
